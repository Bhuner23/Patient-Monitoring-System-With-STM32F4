...
/*
	DİKKAT!!! genellikle Max30102 ve Max30105 modullerinde ir_led ve red_led LED'lerinin kanallari karıştirilir,
	bu nedenle veri okumalari dogru degilse; MAX30102.c ve MAX30102.h dosyasindaki Max30102_ReadFifo fonksiyonunda
	parametreleri değiştirmemiz gerekir. Aşagida bir ornek verilmistir:
...
void Max30102_Task(void)
{
	switch(StateMachine)
	{
		// Parmak sensore yerlestirilmemis. Kırmızı diyot kapali, IR minimumda. Kızılotesi diyot,
		// sensor üzerinde bulunan parmagi algilamak icin tasarlanmiştir. Parmak sensörden kaldırıldığında,
		// program LED'leri kapatarak bu duruma geri doner.
		...
		// Parmağınızı koyarak arabelleği "geçerli veriler" ile doldurun. Varsayılan olarak, hesaplamalar
		// son 5 saniyedeki örnekler üzerinden yapılır. Kalibrasyon, yani tamponu doldurmak, bu yüzden
		// çok uzun sürecektir.
		...
		// Uygun sayıda örnek toplandıktan sonra Maxim algoritmasına göre hesaplamalar yapılır.
		// Bu aşamada, bir saniyeye karşılık gelen örnek sayısına sahip dairesel tampon hareket ettirilir.
		...
		// Sensör hizmeti, bir sonraki numune partisi toplanana kadar, yani bir sonraki saniyeden itibaren
		// bu durumda kalır. Bundan sonra 3 numaralı duruma geçer ve parmak sensörden çıkarılana kadar döngü
		// tekrar eder.
...
MAX30102_STATUS Max30102_Init(I2C_HandleTypeDef *i2c)
{
	...
	// Veri miktarını azaltmak için, bitişik örneklerin (her bir ayrı kanalda) ortalaması alınabilir ve bu kayıt
	// ayarlanarak çip üzerinde kırpılabilir.
	...
	//-- SLOT  - Yalnizca coklu modda ac - ( MODE_MULTI_LED_MODE )------------------------
